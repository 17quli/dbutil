...
select  nvl(cast(admission_wait as int),0) as admission,
	...
	pool,
        ...
        strleft(statement,30) stmt
from dev_raw.mquery
where nvl(cast(admission_wait as int),0)>1000 and from_utc_timestamp(starttime,'NZ') between date_add(now(),interval ${var:v1} minutes) and date_add(now(),interval ${var:v2} minutes)
order by 4 desc
;
